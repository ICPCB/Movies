...
## Next
Gemini round-3 = VERDICT: PASS. Dataset v2 committed @ 320bfbf. Retrain RUNNING in background.
1. When train_intent_lora.py finishes: check final_val_metrics.json, then run `cinematch-llama\.venv\Scripts\python.exe cinematch-llama\scripts\generate_intent_predictions.py`, then `venv\Scripts\python.exe cinematch-llama\scripts\grade_intent_predictions.py`.
2. Gate vs tier-2 numbers in eval/runs/2026-06-11-intent-parser-nogit/report.json — clause (a) validity 1.0, (b) beat tier-2 plot_elements F1 on ALL of plot_description 0.9412 / hybrid 0.7027 / implicit_plot 0.0, (c) no mood regression (tier-1 user_mood_only 0.96, user_and_film 0.95).
3. Ledger gate entry + close lock. Serving integration ONLY after full gate PASS + review.
