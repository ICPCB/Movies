...
- **Human** — final approval, merge, query review, and long-running jobs
  (ingestion, embedding, evaluation runs).
...
13. Claude remains project lead
    - Claude owns planning, architecture, schema contracts, ticket splitting,
      and final review.
    - Codex owns implementation and deterministic validation.
    - If Claude is unavailable due to token limits or rate limits, Codex may
      continue only by following written plans and explicit project rules.
    - Such work must be marked: SELF-REVIEWED / PENDING CLAUDE REVIEW.

14. Codex autonomous approval protocol
    - During long-running automation, Codex may provisionally approve a
      gate/ticket only when:
    ...
    - Every provisional approval must be documented for later Claude review.
    - Codex must not claim final architectural approval.
...
15. Overnight automation protocol
    ...
    - Mark all continued work as pending Claude review.
    - Stop if the next decision requires human movie-label judgment, broad
      architecture change, destructive commands, or unplanned src/* behavior
      changes.
...
2. Report the following back to the human:
